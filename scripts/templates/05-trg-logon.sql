-- Caso o usuário faça logofn do sistema, as estatisticas atuais de sessão
-- são riniciadas para um determinado módulo 
...
BEGIN
  ...
  IF (cur_user <> 'SYS') AND (cur_user <> 'SYSTEM') THEN
    DBMS_APPLICATION_INFO.SET_MODULE (MODULE_NAME => 'TRG_LOGON', ACTION_NAME => 'TRG_LOGON');
