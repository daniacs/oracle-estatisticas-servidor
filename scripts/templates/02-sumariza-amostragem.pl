...
use constant ORA_SRV   =>  '@ORACLE_SRV@';
use constant ORA_INST  =>  '@ORACLE_SID@';
use constant ORA_USR   =>  '@USUARIO@';
...
use constant DEFAULT_TABOUT => 'TANALISE_ESTATISTICAS_HORA';
...
sub ajuda {
  ...
  print "[--prog <LISTA>] [--log <ARQ_LOG>] [--tabin TABIN] [--tabout TABOUT]";
  print "[--truncate] [--debug] [--worktime] [--limit] [--skip n] \n";
...
if (not defined($result->{"TABLE_NAME"})) {
  ...
  print "Tabela $schema.$table_out ja existe\n";
...
# Como a primeira linha nao possui incremento, os valores de
# last_sid e last_event ja podem ser atributidos com os valores atuais.

($rowid, $inst, $last_sid, $prog, $schema, $machine,
  $timestamp, $year, $month, $day, $hour, $min,
  $val, $lastval, $delta, $last_event, $logon_time) = @$result;
...
while ($result = $sth->fetchrow_arrayref()) {
  ...
  $delta = 0 if $delta < 0;
  $timemask = substr($timestamp, 0, 13).":00:00";
  $hashkey = uc($prog)."\t$stat\t$timemask";
  #$hashkey = uc($prog)."\t$stat\t$timemask\t$logon_time";
